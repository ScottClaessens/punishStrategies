...
data {
  ...
  int pred[N];   // predictor
  int nCuts;     // number of simplex cutpoints
...
parameters {
  ...
  simplex[nCuts] sim[10]; // simplexes
}
model {
  // vectors to hold terms of sum, probabilities, and simplex calculations
  vector[10] theta_j;
  vector[10] p;
  vector[10] s;
  ...
  alpha ~ normal(0, 1);
  beta ~ normal(0, 0.5);
  sim[1] ~ dirichlet(rep_vector(2, nCuts));
  sim[2] ~ dirichlet(rep_vector(2, nCuts));
  sim[3] ~ dirichlet(rep_vector(2, nCuts));
  sim[4] ~ dirichlet(rep_vector(2, nCuts));
  sim[5] ~ dirichlet(rep_vector(2, nCuts));
  sim[6] ~ dirichlet(rep_vector(2, nCuts));
  sim[7] ~ dirichlet(rep_vector(2, nCuts));
  sim[8] ~ dirichlet(rep_vector(2, nCuts));
  sim[9] ~ dirichlet(rep_vector(2, nCuts));
  sim[10] ~ dirichlet(rep_vector(2, nCuts));
  ...
  for (B in 1:12) {
    ...
    for (i in 1:N ) {
      ...
      if ( missing==0 ) {
        ...
        for (j in 1:10) s[j] = beta[j] * mo(sim[j], pred[i]);
