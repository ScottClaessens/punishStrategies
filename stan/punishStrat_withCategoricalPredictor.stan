data {
  ...
  array[N] int pred;      // predictor (int in range 1 - J)
...
parameters {
  matrix[J,10] alpha[Nc]; // intercepts for probabilities of different strategies
}
  ...
  for (B in 1:12) {
    ...
    for (i in 1:N ) {
      ...
      if ( missing[B,i]==0 ) {
        ...
        p = softmax( to_vector(alpha[countryID[i]][pred[i],]) );
...
model {
  // priors
  for (i in 1:Nc) to_vector(alpha[i]) ~ normal(0, 1);
