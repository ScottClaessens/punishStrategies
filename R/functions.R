...
loadPilotData <- function(filePilotData) {
  ...
  read_survey(filePilotData) %>%
    ...
    # standardise selfrate vars
    mutate_at(vars(starts_with("SelfRate")), 
              list(std = function(x) as.numeric(scale(x))))
}

# plot pilot results
plotPilotResults1 <- function(pilotPost1) {
  # strategy vector
  strategies <- c("Competitive", "Avoid DI", "Egalitarian", "Seek AI",
                  "Retributive", "Deterrent", "Norm-enforcing", 
                  "Antisocial", "Random choice", "Anti-punish")
  ...
  out <- 
    ...
    theme_classic()
  # save
  ggsave(out, filename = "figures/pilotResults1.pdf", height = 5, width = 6)
  return(out)
}

# plot effect of predictor variable
plotPilotResults2 <- function(dPilot, pilotPost2, predictor, xaxis) {
  # remove na row
  dPilot <- dPilot %>% filter(!is.na(!!as.name(predictor)))
  # x sequence
  x <- seq(min(pull(dPilot, predictor)), max(pull(dPilot, predictor)), length.out = 100)
  # post
  post <- tibble()
  # median and 95% CIs for plotting
  for (i in 1:length(x)) {
    # on logit scale
    p <- pilotPost2$alpha + pilotPost2$beta*x[i]
    # on probability scale
    for (j in 1:nrow(p)) p[j,] <- softmax(p[j,])
    # add to post
    post <- bind_rows(post, tibble(x = x[i], strategy = 1:10, med = apply(p, 2, median),
  ...
  out <-
    ...
    geom_line() +
    facet_wrap(. ~ strategy) +
    labs(x = xaxis, y = "Probability of using strategy") +
    theme_classic()
  # save plot
  ggsave(out, filename = "figures/pilotResults2.pdf", height = 6, width = 6)
  return(out)
}
